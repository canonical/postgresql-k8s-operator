...
async def test_network_cut(ops_test: OpsTest, continuous_writes, master_start_timeout):
    ...
    # assert not await is_connection_possible(ops_test, primary_name), "❌ Connection is possible after network cut"

    await restore_network_for_unit(ops_test, primary_name)

    # wait until network is reestablished for the unit
    # await wait_network_restore(ops_test, primary_name)

    # # verify that connection is possible
    # assert await is_connection_possible(ops_test, primary_name), "❌ Connection is not possible after network restore"
    #
