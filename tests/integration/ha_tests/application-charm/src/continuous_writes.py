...
def continuous_writes(connection_string: str, starting_number: int):
    ...
    while True:
        ...
        f.close()
        try:
            f = open("/tmp/demofile0.0.txt", "a")
            f.write(str(write_value))
            f.write("\n\n")
            f.close()
            with psycopg2.connect(connection_string) as connection, connection.cursor() as cursor:
                f = open("/tmp/demofile0.1.txt", "a")
                f.write(str(write_value))
                f.write("\n")
                f.write(str(connection.status))
                f.write("\n\n")
                f.close()
                connection.autocommit = True
                f = open("/tmp/demofile0.2.txt", "a")
                f.write(str(write_value))
                f.write("\n")
                f.write(str(connection.status))
                f.write("\n\n")
                ...
                f = open("/tmp/demofile0.3.txt", "a")
                f.write(str(write_value))
                f.write("\n")
                f.write(str(connection.status))
                f.write("\n\n")
                f.close()
            f = open("/tmp/demofile0.4.txt", "a")
            f.write(str(write_value))
            f.write("\n")
            f.write(str(connection.status))
            f.write("\n\n")
