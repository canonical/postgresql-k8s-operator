...
# Storage directives for the postgresql-k8s charm: pgdata (the only storage label it defines).
...
def _build_consumer_root(root: Path, module: Path, constraint: str) -> None:
    ...
    (root / "main.tf").write_text(
        ...
        "}\n"
        'output "app_name" {\n'
        "  value = module.postgres.app_name\n"
        "}\n"
    )
...
def test_terraform_apply_deploys_postgresql(juju: jubilant.Juju, tmp_path: Path) -> None:
    ...
    _run_terraform(
        ...
        # Pin the unit to the runner's arch so it schedules there.
        "-var",
    ...
    output = _run_terraform(deploy_dir, TF_TIMEOUT, "output", "-raw", "app_name", capture=True)
