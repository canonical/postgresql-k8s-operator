...
from single_kernel_postgresql.config.locales import LOCALES as ROCK_LOCALES
...
async def test_settings_are_correct(ops_test: OpsTest, unit_id: int):
    ...
    assert settings["data_directory"] == f"{ACTUAL_PGDATA_PATH}"
...
async def test_automatic_failover_after_leader_issue(ops_test: OpsTest) -> None:
    ...
    await ops_test.model.units.get(primary).run(f"rm -rf {ACTUAL_PGDATA_PATH}")
