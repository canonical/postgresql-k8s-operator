...
def test_deploy(first_model: str, second_model: str, charm: str) -> None:
    """Deploy two PostgreSQL clusters with TLS enabled across both, plus test apps.

    K8s equivalent of the VM ``spaces`` async-replication-over-TLS test: Juju spaces are not
    supported on K8s, so this exercises the same path over plain K8s networking. TLS is enabled
    on both clusters (``peer-certificates`` and ``client-certificates``) before they are paired,
    so the cross-model replication set up by the following tests runs over TLS-enabled members.
    """
    ...
    logging.info("Deploying and relating the TLS operator across both clusters")
    model_1.deploy(
        charm=TLS_APP_NAME,
        channel="1/stable",
        base="ubuntu@24.04",
    ...
    retry_if_cli_error(
        lambda: model_1.offer(f"{first_model}.{TLS_APP_NAME}", endpoint="certificates")
    )
    retry_if_cli_error(
        lambda: model_2.consume(f"{first_model}.{TLS_APP_NAME}", "certificates-offer")
    )

    model_1.integrate(f"{DB_APP_1}:client-certificates", TLS_APP_NAME)
    model_1.integrate(f"{DB_APP_1}:peer-certificates", TLS_APP_NAME)
    ...
    retry_if_cli_error(
        lambda: model_1.wait(
            ready=wait_for_apps_status(jubilant.all_active, DB_APP_1, DB_TEST_APP_1, TLS_APP_NAME),
...
def test_async_relate(first_model: str, second_model: str) -> None:
    """Relate the two TLS-enabled PostgreSQL clusters."""
...
def test_data_replication(
    ...
    """Test to write to primary, and read the same data back from replicas over TLS."""
