...
async def test_filling_and_emptying_pgdata_storage(ops_test: OpsTest, charm):
    ...
    await run_command_on_unit(
        ops_test,
        primary,
        f"FREE_SPACE=$(df --output=avail {ACTUAL_PGDATA_PATH} | tail -1) && dd if=/dev/urandom of={ACTUAL_PGDATA_PATH}/tmp bs=1M count=$(( (FREE_SPACE * 91 / 100) / 1024 ))",
    )
    ...
    await run_command_on_unit(ops_test, primary, f"rm {ACTUAL_PGDATA_PATH}/tmp")
