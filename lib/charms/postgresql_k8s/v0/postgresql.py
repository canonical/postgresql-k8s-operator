...
class PostgreSQLListAccessGroupsError(Exception):
    """Exception raised when retrieving PostgreSQL access group list fails."""
...
class PostgreSQL:
    ...
    def _create_access_groups(self) -> None:
    ...
    def _grant_internal_access_group_memberships(self) -> None:
    ...
    def _grant_relation_access_group_memberships(self) -> None:
...
END; $$;"""
    ...
    def list_access_groups(self) -> Set[str]:
        ...
        try:
            with self._connect_to_database() as connection, connection.cursor() as cursor:
                cursor.execute(
                    "SELECT groname FROM pg_catalog.pg_groups WHERE groname LIKE '%_access';"
                )
                access_groups = cursor.fetchall()
                return {groups[0] for groups in access_groups}
        except psycopg2.Error as e:
            logger.error(f"Failed to list PostgreSQL database access groups: {e}")
            raise PostgreSQLListAccessGroupsError() from e
    ...
    def set_up_access_roles(self) -> None:
        """Set up postgres database with the right access roles."""
        access_groups = self.list_access_groups()
        if access_groups == set(ACCESS_GROUPS):
            return

        self._create_access_groups()
        self._grant_internal_access_group_memberships()
    ...
    def update_access_roles(self) -> bool:
        """Whether the update of the PostgreSQL database roles was successful."""
        access_groups = self.list_access_groups()
        if access_groups == set(ACCESS_GROUPS):
            return True
        ...
        try:
            self._create_access_groups()
            self._grant_internal_access_group_memberships()
            self._grant_relation_access_group_memberships()
        except (PostgreSQLAssignGroupError, PostgreSQLCreateGroupError):
            return False
    ...
    def build_postgresql_group_map(group_map: str) -> List[tuple]:
        ...
        for mapping in group_mappings:
            ...
            if len(mapping_parts) != 2:
                raise ValueError("The group-map must contain value pairs split by the equal sign")
