...
def main():
    ...
    print(f"patroni_urls: {patroni_urls}")
    print(f"unit: {unit}")
    ...
    while True:
        ...
        for member in cluster_status["members"]:
            current_authorisation_rules[member["name"]] = member["role"]
        ...
        if is_primary:
            ...
            print(f"current_authorisation_rules 1: {current_authorisation_rules}")
            ...
            # If the authorisation rules changed, dispatch a charm event to handle this change.
                ...
                # }).encode()
                # print(f"data: {data}")
                # req = request.Request(urljoin(patroni_urls.split(",")[0], PATRONI_CONFIG_STATUS_ENDPOINT), data=data)
                ...
                subprocess.run(
                    [
                        "/usr/bin/pebble",
                        "notify",
                        "canonical.com/postgresqlauthorisationruleschange",
                    ],
                    check=True,
                )
