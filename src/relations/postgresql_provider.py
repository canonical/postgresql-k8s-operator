...
class PostgreSQLProvider(Object):
    ...
    def _sanitize_extra_roles(self, extra_roles: str | None) -> list[str]:
        ...
        # Make sure the operator access-group is not in the list
        extra_roles_list = [role.lower() for role in extra_roles.split(",")]
        extra_roles_list = [role for role in extra_roles_list if role != ACCESS_GROUP_OPERATOR]
    ...
    def _on_database_requested(self, event: DatabaseRequestedEvent) -> None:
        ...
        # Make sure the application access-group is added to the list
        extra_user_roles = self._sanitize_extra_roles(event.extra_user_roles)
        extra_user_roles.append(ACCESS_GROUP_APPLICATION)
